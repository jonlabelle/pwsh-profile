    ...
    process
    {
        ...
        foreach ($inputPath in $inputPaths)
        {
            ...
            foreach ($resolvedPath in $resolvedPaths)
            {
                ...
                $itemType = Get-ItemTypeName -Item $item
                $owner = $null
                $group = $null
                $symbolic = $null
                $permissions = $null
                ...
                if ($isUnixPlatform -and $item.PSObject.Properties['UnixStat'])
                {
                    $unixStat = $item.UnixStat
                    $owner = [String]$item.User
                    $group = [String]$item.Group

                    $symbolic = if ($item.PSObject.Properties['UnixMode'])
                    {
                ...
                elseif ($isWindowsPlatform)
                {
                    ...
                    {
                        Write-Verbose "Failed to read ACL for '$($item.FullName)': $($_.Exception.Message)"
                    }
                }
