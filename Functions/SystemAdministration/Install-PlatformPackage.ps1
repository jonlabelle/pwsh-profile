function Install-PlatformPackage
{
    ...
    begin
    {
        ...
        function Select-AvailablePackageRecords
        {
            ...
            $columnWidths = Compress-PackagePickerTableWidths -ColumnWidths $columnWidths -MaximumWidth ($pickerFrameWidth - 4) -IncludesInteractive:$showInteractive
            ...
            try
            {
                ...
                while ($true)
                {
                    ...
                    if ($showInteractive)
                    {
                        $bodyLines += Format-PickerFrameLine -Text ('  {0} {1} {2} {3} {4} {5} {6} {7}' -f 'Sel', (Format-PickerCell -Text 'UI' -Width 3), (Format-PickerCell -Text 'Name' -Width $nameWidth), (Format-PickerCell -Text 'Id' -Width $idWidth), (Format-PickerCell -Text 'Ver' -Width $versionWidth), (Format-PickerCell -Text 'Typ' -Width $typeWidth), (Format-PickerCell -Text 'Src' -Width $sourceWidth), 'Inst') -ForegroundColor DarkGray
                        $bodyLines += Format-PickerFrameLine -Text ('- {0} {1} {2} {3} {4} {5} {6} {7}' -f '---', '---', ('-' * $nameWidth), ('-' * $idWidth), ('-' * $versionWidth), ('-' * $typeWidth), ('-' * $sourceWidth), '----') -ForegroundColor DarkGray
                    }
                    else
                    {
        ...
        function Invoke-PackageInstallation
        {
            ...
            Write-PackageThemeText ''
            Write-PackageThemeText "Installing $targetDescription with $($Manager.DisplayName)..." -ForegroundColor White
    ...
    end
    {
        ...
        switch ($PSCmdlet.ParameterSetName)
        {
            'Query'
            {
                ...
                {
                    Write-PackageThemeText 'No packages matched the requested query.' -ForegroundColor White
                }
                else
                {
                    ...
                    if ($selectedPackages.Count -eq 0)
                    {
                        Write-PackageThemeText 'No packages selected for installation.' -ForegroundColor White
                    }
                }
            }
            'Name'
