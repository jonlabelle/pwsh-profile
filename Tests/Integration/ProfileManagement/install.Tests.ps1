...
Describe 'install.ps1 integration tests' {
    Context 'Git clone install path' {
        It 'clones the repository when LocalSourcePath is not provided' {
            $testRoot = Join-Path -Path $TestDrive -ChildPath ('CloneInstall_{0}' -f ([guid]::NewGuid().ToString('N')))
            $profileRoot = Join-Path -Path $testRoot -ChildPath 'ProfileRoot'
            New-Item -ItemType Directory -Path $profileRoot -Force | Out-Null

            # Seed profile to ensure backup creation
            Set-Content -Path (Join-Path -Path $profileRoot -ChildPath 'preexisting.ps1') -Value '# preexisting'
    ...
    Context 'Local install with default preservation' {
        It 'installs from a local source, preserves default directories, and creates a backup' {
            ...
            {
                $dirPath = Join-Path -Path $profileRoot -ChildPath $name
                New-Item -ItemType Directory -Path $dirPath -Force | Out-Null
                Set-Content -Path (Join-Path -Path $dirPath -ChildPath 'original.txt') -Value "original-$name"
            }
    ...
    Context 'Install via git clone' {
        It 'clones from RepositoryUrl when git is available' -Skip:($null -eq $script:gitExecutable) {
            $testRoot = Join-Path -Path $TestDrive -ChildPath ('GitClone_{0}' -f ([guid]::NewGuid().ToString('N')))
            $profileRoot = Join-Path -Path $testRoot -ChildPath 'ProfileRoot'
            $remoteRepo = Join-Path -Path $testRoot -ChildPath 'RemoteRepo'
            ...
            try
            {
                ...
                $profileFile = Join-Path -Path $remoteRepo -ChildPath 'Microsoft.PowerShell_profile.ps1'
                $modulesDir = Join-Path -Path $remoteRepo -ChildPath 'Modules'
                New-Item -ItemType Directory -Path $modulesDir -Force | Out-Null
                Set-Content -Path $profileFile -Value '# git remote profile'
                Set-Content -Path (Join-Path -Path $modulesDir -ChildPath 'RemoteModule.psm1') -Value 'function Invoke-Remote { "remote" }'
    ...
    Context 'Skip options' {
        It 'does not create backups or preserve directories when flags are provided' {
            $testRoot = Join-Path -Path $TestDrive -ChildPath ('SkipOptions_{0}' -f ([guid]::NewGuid().ToString('N')))
            $profileRoot = Join-Path -Path $testRoot -ChildPath 'ProfileRoot'
            New-Item -ItemType Directory -Path $profileRoot -Force | Out-Null
    ...
    Context 'Restore failure handling' {
        It 'throws when the restore path does not exist' {
            $testRoot = Join-Path -Path $TestDrive -ChildPath ('RestoreFailure_{0}' -f ([guid]::NewGuid().ToString('N')))
            $profileRoot = Join-Path -Path $testRoot -ChildPath 'ProfileRoot'
            $missingBackup = Join-Path -Path $testRoot -ChildPath 'MissingBackup'
            New-Item -ItemType Directory -Path $profileRoot -Force | Out-Null
    ...
    Context 'Skip options' {
        It 'respects SkipBackup and SkipPreserveDirectories switches' {
            ...
            foreach ($name in @('Functions/Local', 'Help', 'Modules', 'PSReadLine', 'Scripts'))
            {
                $dirPath = Join-Path -Path $profileRoot -ChildPath $name
                New-Item -ItemType Directory -Path $dirPath -Force | Out-Null
                Set-Content -Path (Join-Path -Path $dirPath -ChildPath 'test.txt') -Value "test-$name"
            }
    ...
    Context 'Git clone installation' {
        It 'clones from the specified repository when git is available' {
            ...
            try
            {
                ...
                Set-Content -Path (Join-Path -Path $remoteRepo -ChildPath 'cloned.txt') -Value 'cloned content'
                & $script:gitExecutable -C $remoteRepo add . | Out-Null
                & $script:gitExecutable -C $remoteRepo commit -m 'initial commit' | Out-Null
    ...
    Context 'Restore mode' {
        It 'restores profile contents from a supplied backup path without creating a backup by default' {
            $testRoot = Join-Path -Path $TestDrive -ChildPath ('RestoreProfile_{0}' -f ([guid]::NewGuid().ToString('N')))
            $profileRoot = Join-Path -Path $testRoot -ChildPath 'ProfileRoot'
            ...
            Set-Content -Path (Join-Path -Path $profileRoot -ChildPath 'stale.ps1') -Value 'stale profile'
        ...
        It 'creates a backup during restore when BackupPath is explicitly provided' {
            $testRoot = Join-Path -Path $TestDrive -ChildPath ('RestoreWithBackup_{0}' -f ([guid]::NewGuid().ToString('N')))
            $profileRoot = Join-Path -Path $testRoot -ChildPath 'ProfileRoot'
            $backupRoot = Join-Path -Path $testRoot -ChildPath 'BackupSource'
    ...
    Context 'Custom preservation list' {
        It 'only restores directories explicitly requested' {
            $testRoot = Join-Path -Path $TestDrive -ChildPath ('CustomPreserve_{0}' -f ([guid]::NewGuid().ToString('N')))
            $profileRoot = Join-Path -Path $testRoot -ChildPath 'ProfileRoot'
            New-Item -ItemType Directory -Path $profileRoot -Force | Out-Null
    ...
    Context 'Error handling' {
        It 'throws when RestorePath does not exist' {
            $testRoot = Join-Path -Path $TestDrive -ChildPath ('RestoreError_{0}' -f ([guid]::NewGuid().ToString('N')))
            $profileRoot = Join-Path -Path $testRoot -ChildPath 'ProfileRoot'
            New-Item -ItemType Directory -Path $profileRoot -Force | Out-Null
            $missingBackup = Join-Path -Path $testRoot -ChildPath 'MissingBackup'
    ...
    Context 'Skip backup and preservation' {
        It 'does not create backups or restore preserved directories when skipped' {
            $testRoot = Join-Path -Path $TestDrive -ChildPath ('SkipFlags_{0}' -f ([guid]::NewGuid().ToString('N')))
            $profileRoot = Join-Path -Path $testRoot -ChildPath 'ProfileRoot'
            New-Item -ItemType Directory -Path $profileRoot -Force | Out-Null
            ...
            foreach ($name in @('Functions/Local', 'Help', 'Modules', 'PSReadLine', 'Scripts'))
            {
                $dirPath = Join-Path -Path $profileRoot -ChildPath $name
                New-Item -ItemType Directory -Path $dirPath -Force | Out-Null
                Set-Content -Path (Join-Path -Path $dirPath -ChildPath 'keep.txt') -Value "keep-$name"
            }
    ...
    Context 'Error handling' {
        It 'throws when the restore path does not exist' {
            $testRoot = Join-Path -Path $TestDrive -ChildPath ('InvalidRestore_{0}' -f ([guid]::NewGuid().ToString('N')))
            $profileRoot = Join-Path -Path $testRoot -ChildPath 'ProfileRoot'
            $missingRestore = Join-Path -Path $testRoot -ChildPath 'MissingBackup'
    ...
    Context 'Zip download fallback' {
        It 'downloads and extracts repository as zip when git is not available' {
            ...
            try
            {
                ...
                $mockRepoContent = Join-Path -Path $testRoot -ChildPath 'MockRepoContent'
                New-Item -ItemType Directory -Path $mockRepoContent -Force | Out-Null

                # Create mock repository content
                Set-Content -Path (Join-Path -Path $mockRepoContent -ChildPath 'Microsoft.PowerShell_profile.ps1') -Value '# mock profile'
                $mockFunctionsDir = Join-Path -Path $mockRepoContent -ChildPath 'Functions'
                New-Item -ItemType Directory -Path $mockFunctionsDir -Force | Out-Null
                Set-Content -Path (Join-Path -Path $mockFunctionsDir -ChildPath 'Test-Mock.ps1') -Value 'function Test-Mock { "mock" }'
