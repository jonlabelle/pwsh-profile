...
Describe 'MediaProcessing Functions Integration' -Tag 'Integration' {
    ...
    Context 'Default Behavior Verification' {
        ...
        It 'Should demonstrate non-recursive default behavior for all functions' {
            ...
            } -ParameterFilter { $Name -eq 'ffprobe' }
            
            Mock Get-Command { 
                return @{ Path = '/usr/bin/ffmpeg' }
            } -ParameterFilter { $Name -eq 'ffmpeg' }
            
            Mock Test-Path { $true } -ParameterFilter { $Path -like '*ffprobe*' -or $Path -like '*ffmpeg*' }
